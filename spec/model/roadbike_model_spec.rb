...
RSpec.describe Roadbike, type: :model do
  ...
  ride3000_bike_info_hash = {maker_name:"MERIDA", year:2017, bike_series: "Ride",bike_name:"Ride3000",
                             ...
                             bb: "bbだよ", wheel: "FULCRUM/RACING 3", saddle: "オリジナル", seat_pillar:"オリジナル", handle: "オリジナル", stem: "オリジナル",
                             tire: "Panareser/ほげ", pedal: "有", valve: "仏式", accessory: "鍵,リフレクター", maker_url: "http://www.merida.jp/lineup/road_bike/ride_3000.html",
                             shop_url: "http://store.shopping.yahoo.co.jp/cycle-garage1/merida-2017-0004.html",size_list: [50,55,60] ,
                             weight_list: [6.8,6.85,6.9],price: 150000, gear: 22, fork: "FACT carbon fiber, full carbon monocoque, Zertz inserts",
   ...
   defy_adovanced_sl_0_bike_info_hash = {maker_name:"GIANT", year:2017, bike_series: "DEFY",bike_name:"defy_adovanced_sl_0",
                              ...
                              front_derailleur:"フロントだよ",cranc:"クランクだよ",brake:"ブレーキだよ",chain:"チェーンだよ",sprocket:"スプロケだよ",sti_lever:"レバーだよ",
                              bb: "bbだよ", wheel:"FULCRUM/RACING 3",saddle:"オリジナル",seat_pillar:"フレーム一体型",handle:	"GIANT CONTACT SLR Carbon 31.8",stem:"GIANT CONTACT SLR",
 ...
 all_bike_info_hash_list << defy_adovanced_sl_0_bike_info_hash
  ...
  describe "serch_mach_bike(price_up, price_down,sex,road_bike_type,maker_id,frame_type)" do
    ...
    it "detetailが詳細条件のパラメーターが全てnilだった場合" do
      ...
      component_param = nil
      brake_type = nil
      expect(Roadbike.serch_mach_bike(price_up, price_down,sex,road_bike_type,maker_id,frame_type,component_param,brake_type).length).to eq 2
    end
    ...
    it "component_paramで絞れること" do
      ...
      component_param = 14
      brake_type = nil
      expect(Roadbike.serch_mach_bike(price_up, price_down,sex,road_bike_type,maker_id,frame_type,component_param,brake_type).length).to eq 1
    end
    it "メーカーで絞れること" do
  ...
  describe "self.serch_bike_result_and_size(bike_arry, user_size)" do
    ...
    sex = "men"
      it "bike_arryがtrue,user_sizeが１６５だった場合" do
        ...
        component_param = nil
        brake_type = nil
        bike_arry = Roadbike.serch_mach_bike(price_up, price_down,sex,road_bike_type,maker_id,frame_type,component_param,brake_type)
